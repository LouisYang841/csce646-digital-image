...
//   uSunPos         太阳位置     -1 .. 2.2 （页面侧会自动递增；> 1 表示已升出画面上沿）
...
uniform float uWarmTemp;
uniform float uCoolTemp;

// fbm 的细节层数：uniform 是浮点，转成 int 用
#define DETAIL int(uNoiseDetail + 0.5)
...
vec4 foreground(vec2 uv, float t){
    ...
    h = (fbm(uv2, DETAIL) - 0.5)*disp*uCloudShape;
...
vec3 vignettCorrection( vec3 col, vec2 uv )
{
    ...
    // 昼夜循环 ////////////////////////////////////////////////////////////////
    // 原版这里是 1.3 + 0.4*cos(0.5*(iTime+3.14))，一个纯靠时间驱动的呼吸效果。
    // 现在改成跟随太阳位置：
    //   - 太阳在画面里（uSunPos 从 -1 升到 ~1）时是白天
    //   - 太阳升出画面上沿（uSunPos > 1，相当于转到地球另一侧）之后逐渐变暗
    //   - 两端取同一个亮度（0.30），所以位置回卷时不会跳变
    // 初值 uSunPos = 0.17 时因子正好是 1.30，与原版 t=0 的画面一致。
    float day = 1.0 - smoothstep(0.95, 1.9, uSunPos);   // 1 = 白天，0 = 入夜
    float dawn = smoothstep(-1.0, 0.1, uSunPos);        // 刚升起时偏暗
    col1 *= 0.30 + 1.00 * day * dawn;
...
export default landscapeSource;
